  ...
  async fetch(request) {
    ...
    const fetchYahoo = async (sym) => {
      try {
        ...
        if (!r) return { price: null, error: "Symbol Not Found" };
        return {
    ...
    if (url.pathname === "/cnn") {
      try {
        ...
        const j = await res.json();
        resData = { price: parseInt(j.data[0].value), label: j.data[0].value_classification, error: null };
      } catch (e) { resData.error = e.message; }
    } 
    else if (url.pathname === "/vix") resData = await fetchYahoo("^VIX");
    else if (url.pathname === "/vixtw") resData = await fetchYahoo("^VIXTW");
      ...
      try {
        ...
        resData = { price: last.close, change: change, isUp: change >= 0, error: null };
      } catch (e) { resData.error = e.message; }
    }
